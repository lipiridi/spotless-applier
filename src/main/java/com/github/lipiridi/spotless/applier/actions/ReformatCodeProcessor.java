...
public class ReformatCodeProcessor extends AbstractLayoutCodeProcessor {
     ...
     *     flag set
     */
    @NotNull private ExternalSystemTaskExecutionSettings constructTaskExecutionSettings(PsiFile fileToProcess) {
        ...
        if (!isReformatAllEnabled) {
            settings.setScriptParameters(String.format(
                    "-PspotlessIdeHook=\"%s\"%s", fileToProcess.getVirtualFile().getPath(), noConfigCacheOption));
        }
    ...
    @Nullable private static PsiFile ensureValid(@NotNull PsiFile file) {
        if (file.isValid()) {
            return file;
        }
    ...
    private static void assertFileIsValid(@NotNull PsiFile file) {
        if (!file.isValid()) {
            LOG.error("Invalid Psi file, name: "
                    + file.getName()
                    + " , class: "
