...
public class SpotlessOnSaveOptionsBase<S extends SpotlessOnSaveOptionsBase.StateBase>
   ...
   * 'Actions on Save' page, unless implementations of this interface specify a different behavior.
   * In other words, this extension point allows having 'Reformat code' and/or 'Optimize imports'
   * enabled out-of-the-box for the specified file types.
   */
  ...
  static class StateBase implements Cloneable {
    StateBase(
        @NotNull
            Function<? super DefaultsProvider, ? extends Collection<FileType>>
                enabledByDefaultProvider) {
      ...
      if (mySelectedFileTypes.isEmpty()) {
        // Default state when there are no extensions that want some files to be processed on save
        // by default.
        myRunOnSave = false;
    ...
    /**
     * If <code>true</code> then {@link #mySelectedFileTypes} doesn't matter. This field set to
     * <code>true</code> guarantees that 'Run this action on save' option remains enabled for all
     * file types even if newly installed plugins contribute new file types (which obviously not yet
     * listed in {@link #mySelectedFileTypes}).
     */
    public boolean myAllFileTypesSelected;

    /**
     * Contains file types (names) that have been individually selected to be processed on save.
     * Makes sense only if {@link #myAllFileTypesSelected} is <code>false</code>.
     */
  ...
  public SpotlessOnSaveOptionsBase(@NotNull S state) {
    myState = state;
  }
  ...
  /**
   * Returns {@code true} if either the root `All file types` check box is checked or the root check
   * box is unchecked but the specific `fileType` check box is checked. <br>
   * <br>
   * Note that most of the callers should first call {@link #isRunOnSaveEnabled()}. The fact that
   * `All file types` or a specific file type is checked doesn't automatically mean that the feature
   * itself ('Run this action on save') is enabled.
   */
  public boolean isFileTypeSelected(@NotNull FileType fileType) {
    return myState.myAllFileTypesSelected
        || myState.mySelectedFileTypes.contains(fileType.getName());
  }
  ...
  @Override
  public SpotlessOnSaveOptionsBase<S> clone() {
    try {
      //noinspection unchecked
      SpotlessOnSaveOptionsBase<S> clone = (SpotlessOnSaveOptionsBase<S>) super.clone();
      //noinspection unchecked
  ...
  public boolean equals(Object o) {
    if (this == o) return true;
    if (o == null || getClass() != o.getClass()) return false;
    SpotlessOnSaveOptionsBase<?> base = (SpotlessOnSaveOptionsBase<?>) o;
