...
public abstract class SpotlessOnSaveActionInfoBase<Options extends SpotlessOnSaveOptionsBase<?>>
  ...
  public @Nullable ActionOnSaveComment getComment() {
    if (myCurrentUiState.isRunOnSaveEnabled()
      ...
      return ActionOnSaveComment.warning(
          CodeInsightBundle.message("actions.on.save.page.warning.no.file.types.selected"));
    }
  ...
  @NotNull
  DropDownLink<String> createFileTypesDropDownLink() {
    Function1<DropDownLink<String>, JBPopup> popupBuilder = this::createFileTypesPopup;
  ...
  private @NotNull @NlsContexts.LinkLabel String getFileTypesLinkText() {
    ...
    if (fileTypes.size() == 1) {
      return CodeInsightBundle.message(
          "actions.on.save.page.label.one.file.type.selected", presentableFileType);
    }

    return CodeInsightBundle.message(
        "actions.on.save.page.label.many.file.types.selected",
        presentableFileType,
        fileTypes.size() - 1);
  }

  private @NotNull JBPopup createFileTypesPopup(@NotNull DropDownLink<String> link) {
    CheckedTreeNode root =
        new CheckedTreeNode(CodeInsightBundle.message("actions.on.save.page.label.all.file.types"));

    SortedSet<FileType> result =
        new TreeSet<>(
            Comparator.comparing(FileType::getDescription, String.CASE_INSENSITIVE_ORDER));
    ...
    return JBPopupFactory.getInstance()
        .createComponentPopupBuilder(new JBScrollPane(tree), tree)
  ...
  private @NotNull CheckboxTree createFileTypesCheckboxTree(@NotNull CheckedTreeNode root) {
    CheckboxTree tree =
        new CheckboxTree(createFileTypesRenderer(), root) {
          @Override
  ...
  private void onFileTypePopupClosed(
      @NotNull DropDownLink<String> link, @NotNull CheckedTreeNode root) {
